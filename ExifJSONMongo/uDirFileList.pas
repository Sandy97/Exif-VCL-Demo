...
type
  ...
  private
    procedure DoNotification(const noName, noTitle, noMsg: string);
...
//          ToggleSwitch1.State:=tssOff;
  ...
  DoNotification('DirFilesListDemo','Загрузка в MongoDB',format('Загружено %d файлов из %s',
    [ListBox2.Items.Count,  ListBox1.Items[ListBox1.ItemIndex] ]));
  sw.Stop;
  StatusBar1.SimpleText := 'msec: ' + sw.ElapsedMilliseconds.ToString;
end;

procedure TfmDirFileList.DoNotification(const noName, noTitle, noMsg: string);
var
  MyNotification: TNotification;
begin
  MyNotification := NotificationCenter1.CreateNotification;
  try
    MyNotification.Name := noName;
    MyNotification.Title := noTitle;
    MyNotification.AlertBody := noMsg;
...
procedure TfmDirFileList.btVolInfoClick(Sender: TObject);
var
  Drive: String;
  DriveLetter: String;
